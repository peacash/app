	...
	<div class="flex flex-col gap-2 md:gap-10 my-2 md:my-10 w-full">
		<Description>
			<Table v-if="dynamic !== null">
				<div v-if="!dynamic.random_queue.length" class="text-center italic">No stakers.</div>
				<TableRow v-for="(address, index) in dynamic.random_queue" :key="(hash, index)">
					<TD1>#{{ index }}</TD1>	
